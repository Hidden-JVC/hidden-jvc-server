...
noPasswordBackupFilename="backups/HiddenJVC_$(date +%Y-%m-%d"_"%H-%M-%S)_No_Passwords.sql"
...
cat tools/removeUsersPasswords.sql | docker exec -i hidden-database psql -U hidden-jvc-user -d HiddenJVCBackups
